...
if __name__ == '__main__':
    ...
    if args.model in ['forward_model', 'inverse_model']:
        ...
        print(model)
        optimizer_G = torch.optim.Adam(model.generator.parameters(), lr=configs['g_learning_rate'], betas=(0.5, 0.999), weight_decay=configs['weight_decay'])
        optimizer_D = torch.optim.Adam(model.discriminator.parameters(), lr=configs['d_learning_rate'], betas=(0.5, 0.999),  weight_decay=configs['weight_decay'])
        ...
        trainer = GANTrainer(model, forward_model, optimizer_G, optimizer_D, train_loader, val_loader, test_loader, criterion, configs['epochs'], args.model, configs['n_critic'], configs['clip_value'])
        trainer.fit()
        sys.exit(0)
