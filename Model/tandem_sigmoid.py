...
# To speficy which GPU to use, run with: CUDA_VISIBLE_DEVICES=5,6 python  tandem_sigmoid.py
...
def main(configs):
    ...
    forward_model_evaluate.load_state_dict(torch.load('./models/forward_model_trained_evaluate_1.pth')['model_state_dict'])
    ...
    # start training 
    path =  './models/tandem_sigmoid/tandem_sigmoid'+'_lr_'+str(configs.lr)+'_STEP_'+ str(configs.if_lr_de) +'_trained.pth'
    path_temp = './models/tandem_sigmoid/tandem_sigmoid'+'_lr_'+str(configs.lr)+'_STEP_'+ str(configs.if_lr_de) +'_trained_temp.pth'
...
if __name__  == '__main__':
    parser = argparse.ArgumentParser('nn models for inverse design: tandem_sigmoid')
    parser.add_argument('--model', type=str, default='tandem_sigmoid')
